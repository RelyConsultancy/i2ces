...
const TableCusts = ({ data }) => {
  ...
  const rows = [TR(
    TD('During'),
    TD(fmtUnit(data.during.uplift, 'units')),
  ...
  if (data.post.uplift) {
    rows.push(TR(
      TD('Post'),
      TD(fmtUnit(data.post.uplift, 'units')),
  ...
  const footer = TR(
    ...
    TD('Total'),
    TD(fmtUnit(data.total.uplift, 'units')),
...
export default Component({
  ...
  render () {
    ...
    return B({ className: style.component }, TableCusts({ data }))
  }
})