...
const TableSales = ({ data }) => {
  ...
  const rows = [TR(
    ...
    TD(fmtUnit(data.during.percentage_uplift * 100, 'percentage'))
  )]
  ...
  if (data.post.uplift) {
    rows.push(TR(
      ...
      TD(fmtUnit(data.post.percentage_uplift * 100, 'percentage'))
    ))
  }
  ...
  const footer = TR(
    ...
    TD(fmtUnit(data.total.percentage_uplift * 100, 'percentage'))
  )
