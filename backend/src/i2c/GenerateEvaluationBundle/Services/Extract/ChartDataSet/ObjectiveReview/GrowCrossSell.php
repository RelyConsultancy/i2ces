 ...
 */
class GrowCrossSell implements ExtractInterface
{
    /** @var Connection */
    ...
    public function getOfferData($cid)
    {
        return sprintf(
            'SELECT timeperiod AS timeperiod, exposed AS exposed, control AS control
             ...
             AND metric=\'GCS_custs\'
             AND objective=\'Grow customer product range (cross sell)\';
            ',
            $cid
        );
    }
    ...
    public function getDuringCombinations($cid)
    {
        return sprintf(
            'SELECT r.media_type AS media_type, r.uplift AS uplift, r.pct_uplift AS percentage_uplift, r.control AS control, e.exposed AS exposed
             ...
             AND r.timeperiod=2
             AND r.metric=\'GCS_custs\'
             AND r.objective=\'Grow customer product range (cross sell)\'
             AND r.product=\'Offer\'
    ...
    public function getObjectivePriority ($cid)
    {
        return sprintf(
                'SELECT DISTINCT obj_priority from ie_results_data
                 WHERE objective=\'Grow customer product range (cross sell)\'
